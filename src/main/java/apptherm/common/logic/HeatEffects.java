...
public class HeatEffects {
	...
	protected void StageThree() {
		...
		if (getMeltTicker() <= 0) {
			...
			machineWorld.setBlock(machine.x, machine.y, machine.z, BlockInfo.LIQUID_ME_ID);
			setMeltTicker(10000);
		}
		decrMeltTicker();
	...
	protected void StageTwo() {
		...
		if (getParticleTicker() == 5) {
			...
			PacketDistributer.sendEffectPacket((byte) 1, tile.x, tile.y, tile.z,
	...
	protected void StageOne() {
		...
		if (getParticleTicker() <= 0) {
			...
			PacketDistributer.sendEffectPacket((byte) 0, tile.x, tile.y, tile.z,
					tile.getCoord().getWorld().provider.dimensionId);
	...
	public static void receiveEntityFXPacket(byte effectId, int x, int y, int z) {
		...
		case 0:
			for (int i = 0; i < 15; i++) {
				...
				Minecraft.getMinecraft().effectRenderer.addEffect(new EntitySmokeFX(world, (double) (xCoord + xRandom),
						yCoord, (double) (zCoord + zRandom), 0.0, 0.0, 0.0));
			}
			break;
		case 1:
			for (int i = 0; i < 15; i++) {
				float xRandom = rnd.nextFloat() * 0.6F - 0.3F;
				float zRandom = rnd.nextFloat() * -0.6F - -0.3F;
				Minecraft.getMinecraft().effectRenderer.addEffect(new EntityFlameFX(world, (double) (xCoord + xRandom),
						yCoord, (double) (zCoord + zRandom), 0.0, 0.0, 0.0));
			}
